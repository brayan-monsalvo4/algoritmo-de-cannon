...
int main(int argc, char* argv[]) {
    ...
    char nuc[2];
    sprintf(nuc, "%d", NUCL);
    char *filename = malloc(strlen(nuc) + strlen("f_100sec") + 1);
    strcpy(filename, nuc);
    strcat(filename, "f_100sec_");
    ...
    char *filename4 = malloc(strlen(nuc) + strlen("f_PTsec_") + 1);
    strcpy(filename4, nuc);
    strcat(filename4, "f_PTsec");
